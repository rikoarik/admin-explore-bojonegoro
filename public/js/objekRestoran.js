...
class RestoranItem {
  constructor({ no, Restoran, alamat, latitude, longitude, imageUrl, kategori, deskripsi, lainLain, fasilitas }) {
    this.no = no;
...
class AppController {
  ...
  displayData() {
    ...
    onValue(ref(database, 'Restoran/'), (snapshot) => {
      const data = snapshot.val();
      dataTableBody.innerHTML = '';
      for (const key in data) {
        if (data.hasOwnProperty(key)) {
          const item = new RestoranItem(data[key]);
  ...
  deleteRow(key) {
    ...
    const itemRef = ref(database, `Restoran/${key}`);
    set(itemRef, null)
      .then(() => {
  ...
  submitAddForm() {
    const formData = {
      ...
      Restoran: document.getElementById('Restoran').value,
    ...
    const newPostRef = push(ref(database, 'Restoran/'));
  ...
  submitEditForm() {
    ...
    set(ref(database, `objekRestoran/${this.selectedKey}`), formData)
      .then(() => {
  ...
  getEditFormData() {
    return {
      ...
      Restoran: document.getElementById('editRestoran').value,
